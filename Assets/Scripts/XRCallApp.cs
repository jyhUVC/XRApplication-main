...
public class XRCallApp : MonoBehaviour
{
    /// <summary>
    /// This is a test server. Don't use in production! The server code is in a zip file in WebRtcNetwork
    /// </summary>
    [SerializeField] private string uSignalingUrl = "ws://signaling.because-why-not.com/callapp";

    //ws://signaling.because-why-not.com/callapp
    ...
    /// </summary>
    [SerializeField] private string uSecureSignalingUrl = "wss://signaling.because-why-not.com/callapp";
    //wss://signaling.because-why-not.com/callapp


    /// <summary>
    /// If set to true only the secure signaling url will be used.
    /// </summary>
    [SerializeField] private bool uForceSecureSignaling = false;
    ...
    /// method.
    /// </summary>
    [SerializeField] private string uIceServer = "stun:stun.because-why-not.com:443";

    //
    [SerializeField] private string uIceServerUser = "";
    [SerializeField] private string uIceServerPassword = "";
    ...
    /// </summary>
    [SerializeField] private string uIceServer2 = "stun:stun.l.google.com:19302";

    /// <summary>
    /// Do not change. This length is enforced on the server side to avoid abuse.
    /// </summary>
    [SerializeField] private const int MAX_CODE_LENGTH = 256;
    ...
    [SerializeField] private int idealFPS = 30;

    protected virtual void Awake()
    {
        xrCallUI = GetComponent<XRCallUI>();
    ...
    public virtual void SetupCall()
    {
        ...
        mMediaConfigInUse.Audio = false;// 안되면 false로 되돌리기@@
    ...
    public virtual MediaConfig CreateMediaConfig()
    {
        ...
        mediaConfig.MinWidth = Screen.width / 4;
        mediaConfig.MinHeight = Screen.height / 4;
        ...
        //encode fast enough.
        mediaConfig.MaxWidth = Screen.width;
        mediaConfig.MaxHeight = Screen.height;

        //will be overwritten by UI in normal use
        mediaConfig.IdealWidth = Screen.width / 4;
        mediaConfig.IdealHeight = Screen.height / 4;
        mediaConfig.IdealFrameRate = idealFPS;
        return mediaConfig;
    }
    ...
    protected virtual void CleanupCall()
    {
        if (mCall != null)
        {
            ...
            xrCallUI.ChangeUIRemotePeerState(false);
        }
    }
    ...
    protected virtual void UpdateFrame(FrameUpdateEventArgs frameUpdateEventArgs)
    {
        ...
        {
            xrCallUI.UpdateLocalTexture(frameUpdateEventArgs.Frame, frameUpdateEventArgs.Format);
        }
        else
        {
            xrCallUI.UpdateRemoteTexture(frameUpdateEventArgs.Frame, frameUpdateEventArgs.Format);
        }
    }
